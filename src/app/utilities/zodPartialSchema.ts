 ...
 */
export function createPartialSchema<TShape extends ZodShape>(schema: ZodObject<TShape>) {
	const shape = schema.shape;

	const partialShape = Object.fromEntries(
		Object.entries(shape).map(([k, field]) => [k, deepPartial(field).optional()])
	) as ZodDeepPartial<TShape>;
...
export type Deepen<T extends ZodType> =
	T extends ZodObject<infer S extends Record<string, ZodType>> ?
...
export type ZodDeepPartial<TShape extends Record<string, ZodType>> = {
	[K in keyof TShape]: ZodOptional<Deepen<TShape[K]>>;
};
